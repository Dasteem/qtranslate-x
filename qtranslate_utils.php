...
if (WP_DEBUG) {
    if (!function_exists('qtranxf_dbg_log')) {
        ...
        function qtranxf_dump($var, $highlight = true) {
            $output = qtranxf_dumpInternal($var, 0);
            if ($highlight) {
        ...
        function qtranxf_dumpInternal($var, $level) {
            $output = '';
            $objects = array();
            switch (gettype($var)) {
                case 'boolean':
                ...
                case 'array':
                    if (empty($var))
                        ...
                        foreach ($keys as $key) {
                            $output.="\n" . $spaces . '    ';
                            $output.= qtranxf_dumpInternal($key, 0);
                            $output.=' => ';
                            $output.= qtranxf_dumpInternal($var[$key], $level + 1);
                        }
                        $output.="\n" . $spaces . ')';
                    }
                    break;
                case 'object':
                    if (($id = array_search($var, $objects, true)) !== false)
                        ...
                        foreach ($members as $key => $value) {
                            ...
                            $output.="\n" . $spaces . "    [$keyDisplay] => ";
                            $output.= qtranxf_dumpInternal($value, $level + 1);
                        }
                        $output.="\n" . $spaces . ')';
                    }
                    break;
